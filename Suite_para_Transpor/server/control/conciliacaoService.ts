...
export async function conciliarLancamento(
  ...
  return await db.transaction(async (tx) => {
    ...
    const movExistRows = await tx.execute<any>(sql`
      ...
      WHERE tenant_id = ${tenantId} AND lancamento_financeiro_id = ${lancamentoId}
      FOR UPDATE
    `);
...
export async function getExtratoConta(
  ...
  // ── UNION: movimentações bancárias + lançamentos vinculados diretamente ──
  // Lançamentos com conta_bancaria_id + status pago/conciliado que ainda não
  // possuem linha em movimentacoes_bancarias aparecem diretamente no extrato.
  const dataIniParam = filtros.dataIni ?? null;
  const dataFimParam = filtros.dataFim ?? null;

  const result = await db.execute(sql`
    SELECT
      mb.id,
      mb.tenant_id   AS "tenantId",
      mb.conta_bancaria_id AS "contaBancariaId",
      mb.lancamento_id     AS "lancamentoId",
      mb.data,
      mb.tipo,
      mb.origem,
      mb.descricao,
      mb.valor,
      mb.saldo_apos   AS "saldoApos",
      mb.created_at   AS "createdAt",
      lf.descricao    AS "lancamentoDescricao"
    FROM movimentacoes_bancarias mb
    LEFT JOIN lancamentos_financeiros lf ON lf.id = mb.lancamento_id
    WHERE mb.tenant_id = ${tenantId}
      AND mb.conta_bancaria_id = ${contaBancariaId}
      AND (${dataIniParam}::date IS NULL OR mb.data >= ${dataIniParam}::date)
      AND (${dataFimParam}::date IS NULL OR mb.data <= ${dataFimParam}::date)
    ...
    SELECT
      lf2.id,
      lf2.tenant_id,
      lf2.conta_bancaria_id,
      lf2.id          AS "lancamentoId",
      COALESCE(lf2.data_pagamento, lf2.data_vencimento) AS data,
      CASE WHEN lf2.tipo = 'receber' THEN 'entrada' ELSE 'saida' END AS tipo,
      COALESCE(lf2.origem, 'manual') AS origem,
      lf2.descricao,
      lf2.valor,
      NULL::numeric   AS "saldoApos",
      lf2.created_at  AS "createdAt",
      lf2.descricao   AS "lancamentoDescricao"
    FROM lancamentos_financeiros lf2
    WHERE lf2.tenant_id = ${tenantId}
      AND lf2.conta_bancaria_id = ${contaBancariaId}
      AND lf2.status IN ('pago', 'conciliado')
      AND (${dataIniParam}::date IS NULL OR COALESCE(lf2.data_pagamento, lf2.data_vencimento) >= ${dataIniParam}::date)
      AND (${dataFimParam}::date IS NULL OR COALESCE(lf2.data_pagamento, lf2.data_vencimento) <= ${dataFimParam}::date)
      AND lf2.id NOT IN (
        SELECT lancamento_id FROM movimentacoes_bancarias
  ...
  const movs: any[] = (result as any).rows ?? (result as any) ?? [];

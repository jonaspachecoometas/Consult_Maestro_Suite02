...
import { GrupoEmpresarialSelector, GrupoBanner } from "@/components/GrupoEmpresarialSelector";
import { useGrupoEmpresarial } from "@/hooks/useGrupoEmpresarial";
...
import { useSoeMotor, SoeMotor } from "@/contexts/SoeMotorContext";
import { ComprasTab } from "./soe/ComprasTab";
import { EstoqueTab } from "./soe/EstoqueTab";
import { IntegracaoTab } from "./soe/IntegracaoTab";

type TabType = "dashboard" | "persons" | "customers" | "suppliers" | "products" | "sales" | "purchases" | "compras_soe" | "estoque" | "crm" | "accounting" | "projects" | "hr" | "config" | "sync" | "integracao";
...
function PersonsTab() {
  ...
  const handleSave = async () => {
    try {
      ...
      const url = editingPerson ? `/api/soe/persons/${editingPerson.id}` : "/api/soe/persons";
...
export default function SOE() {
  ...
  const { motor: erpProfile, setMotor: setErpProfile } = useSoeMotor();

  const tenantId = (user as any)?.tenantId || 1;

  // SOE-02: Queries nativas via tabela pessoas (fonte canônica)
  const { data: pessoasClientes = [] } = useQuery<any[]>({
    queryKey: ["/api/soe/persons", "customer"],
    queryFn: () => api.get("/api/soe/persons?roleFilter=customer"),
  });

  const { data: pessoasFornecedores = [] } = useQuery<any[]>({
    queryKey: ["/api/soe/persons", "supplier"],
    queryFn: () => api.get("/api/soe/persons?roleFilter=supplier"),
  });

  const customers: Customer[] = (pessoasClientes || []).map((p: any) => ({
    id: p.id,
    code: String(p.id).slice(0, 8),
    name: p.fullName || "",
    type: p.cpfCnpj?.replace(/\D/g, "").length > 11 ? "PJ" : "PF",
    taxId: p.cpfCnpj || "",
    email: p.email || "",
    phone: p.phone || "",
    city: p.city || "",
  ...
  const suppliers: Supplier[] = (pessoasFornecedores || []).map((p: any) => ({
    id: p.id,
    code: String(p.id).slice(0, 8),
    name: p.fullName || "",
    type: p.cpfCnpj?.replace(/\D/g, "").length > 11 ? "PJ" : "PF",
  ...
  const { data: localProducts = [] } = useQuery<Product[]>({
    queryKey: ["/api/soe/products"],
    queryFn: () => api.get("/api/soe/products"),
  });
  ...
  const { data: segments = [] } = useQuery<ErpSegment[]>({
    queryKey: ["/api/soe/segments"],
    queryFn: () => api.get("/api/soe/segments"),
  });

  const { data: erpConfigFromServer } = useQuery<ErpConfigData | null>({
    queryKey: ["/api/soe/config"],
    queryFn: () => api.get("/api/soe/config"),
  });
  ...
  const handleSeedSegments = async () => {
    try {
      await api.post("/api/soe/segments/seed", {});
      queryClient.invalidateQueries({ queryKey: ["/api/soe/segments"] });
  ...
  const handleSaveConfig = async () => {
    try {
      await api.post("/api/soe/config", erpConfigData);
      queryClient.invalidateQueries({ queryKey: ["/api/soe/config"] });
  ...
  const { data: purchaseOrders = [] } = useQuery<PurchaseOrder[]>({
    queryKey: ["/api/soe/purchase-orders", tenantId],
    queryFn: () => api.get(`/api/soe/purchase-orders?tenantId=${tenantId}`),
  });
  ...
  const displaySuppliers = stats.suppliers || suppliers.length;
  const displayProducts = stats.products || products.length;
  const displaySalesOrders = stats.salesOrders || salesOrders.length;
  const displayPurchaseOrders = stats.purchaseOrders || purchaseOrders.length;
  ...
  const batchAddDevicesMutation = useMutation({
    mutationFn: ({ productId, devices }: { productId: number; devices: Array<{ imei: string }> }) => 
      api.post(`/api/soe/products/${productId}/devices/batch`, { devices }),
    onSuccess: (data: any) => {
      queryClient.invalidateQueries({ queryKey: ["/api/soe/products"] });
  ...
  const generateNfeMutation = useMutation({
    mutationFn: (id: number) => api.post(`/api/soe/sales-orders/${id}/generate-nfe`, {}),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/soe/sales-orders"] });
      ...
      <div className="h-full flex flex-col bg-background">
        <div className="border-b bg-muted/30 px-6 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-xl font-bold">Arcádia SOE</h1>
                <p className="text-sm text-muted-foreground">Sistema Operacional Empresarial</p>
              </div>
            </div>
            <div className="flex items-center gap-2">
              <GrupoEmpresarialSelector />
          ...
          <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as TabType)} className="h-full flex flex-col">
            <div className="border-b px-6">
              <TabsList className="h-12">
                ...
                <TabsTrigger value="compras_soe" className="gap-2" data-testid="tab-compras-soe">
                  <Truck className="w-4 h-4" />
                  Compras NF-e
                </TabsTrigger>
                <TabsTrigger value="estoque" className="gap-2" data-testid="tab-estoque">
                  <Warehouse className="w-4 h-4" />
                  Estoque
                </TabsTrigger>
                <TabsTrigger value="integracao" className="gap-2" data-testid="tab-integracao">
            ...
            <ScrollArea className="flex-1 p-6">
                  ...
                  <CardContent>
                    <div className="grid grid-cols-3 gap-4">
                      ...
                      <div className="p-4 border rounded-lg">
                        ...
                        <Button variant="outline" size="sm" className="w-full" onClick={() => window.location.href = "/hr"}>
                          Acessar RH
                        </Button>
                      </div>
                    </div>
              ...
              <TabsContent value="compras_soe" className="mt-0">
                <ComprasTab />
              </TabsContent>
              ...
              <TabsContent value="config" className="mt-0 space-y-6">
                <div className="flex items-center justify-between">
                  <div>
                    <h2 className="text-2xl font-bold">Configuração SOE</h2>
                    <p className="text-muted-foreground">Configure o SOE de acordo com seu segmento de negócio</p>
                  </div>
        ...
        <Dialog open={showProductDialog} onOpenChange={(open) => { setShowProductDialog(open); if (!open) setEditingProduct(null); }}>
          <DialogContent className="max-w-2xl max-h-[90vh] flex flex-col">
            ...
            <form onSubmit={handleSaveProduct} className="flex-1 overflow-y-auto pr-2">
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-2">
                  <Label htmlFor="code">Código</Label>
...
function SyncDashboard() {
  ...
  const { data: syncStatus, isLoading: loadingStatus } = useQuery({
    ...
    queryFn: async () => {
      const res = await fetch("/api/soe/sync/status", { credentials: "include" });
  ...
  const syncMutation = useMutation({
    mutationFn: async (type: string) => {
      const endpoint = type === "persons" ? "/api/retail/plus/sync/customers" :
                       type === "sales" ? "/api/retail/plus/sync/sales" :
                       `/api/retail/plus/sync/${type}`;
      const body: Record<string, any> = {};
      if (type === "persons") body.tenantId = tenantModules?.tenantId || 1;
      if (type === "sales") body.saleId = 0;
      ...
      if (!res.ok) {
        ...
        throw new Error(err.error || err.action || "Sync failed");
      }
      return res.json();
    },
    onSuccess: (data, type) => {
      toast({ title: "Sincronização concluída", description: `${type}: ${data.created || 0} criados, ${data.updated || 0} atualizados` });
  ...
  const features = tenantModules?.features || {};

  const motors = [
    {
      id: "retail",
      name: "Retail → SOE",
      description: "Sincroniza vendas do PDV, clientes e produtos com o SOE",
      icon: <Building2 className="w-6 h-6" />,
      enabled: features.retail,
      color: "from-orange-500 to-red-500",
      syncActions: [
        { label: "Sincronizar Pessoas", type: "persons", icon: <Users className="w-4 h-4" /> },
        { label: "Sincronizar Vendas", type: "sales", icon: <TrendingUp className="w-4 h-4" /> },
      ]
    },
    {
      id: "plus",
      name: "Plus (Motor ERP)",
      description: "Motor de execução ERP Laravel - NF-e, Estoque, Financeiro",
      icon: <Database className="w-6 h-6" />,
      enabled: features.plus,
      color: "from-violet-600 to-purple-600",
      syncActions: [
        { label: "Sincronizar Clientes", type: "persons", icon: <Users className="w-4 h-4" /> },
        { label: "Sincronizar Vendas", type: "sales", icon: <TrendingUp className="w-4 h-4" /> },
      ]
    },
    {
      id: "fisco",
      name: "Motor Fiscal",
      description: "Compliance tributário - NF-e, NFC-e, NCM, CFOP",
      icon: <Receipt className="w-6 h-6" />,
      enabled: features.fisco,
      color: "from-emerald-600 to-emerald-800",
      syncActions: []
    },
    {
      id: "bi",
      name: "Motor BI (Insights)",
      description: "Business Intelligence - Dashboards e análises de dados",
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold" data-testid="text-sync-title">Sincronização de Motores</h2>
          <p className="text-muted-foreground">
            Gerencie a sincronização entre o SOE e os motores de execução.
            Ative módulos em Admin → Módulos.
          </p>
        </div>
        <Badge variant="outline" className="text-sm">
          {motors.filter(m => m.enabled).length}/{motors.length} motores ativos
        </Badge>
      </div>

      <div className="grid gap-4 md:grid-cols-2">
        {motors.map((motor) => (
          <Card key={motor.id} className={`relative ${!motor.enabled ? 'opacity-60' : ''}`} data-testid={`sync-card-${motor.id}`}>
            <CardHeader className="pb-3">
              <div className="flex items-center gap-3">
                <div className={`p-3 rounded-xl bg-gradient-to-br ${motor.color} text-white`}>
                  {motor.icon}
                </div>
                <div className="flex-1">
                  <CardTitle className="text-lg flex items-center gap-2">
                    {motor.name}
                    {motor.enabled ? (
                      <Badge className="bg-emerald-100 text-emerald-700 text-xs" data-testid={`badge-enabled-${motor.id}`}>Ativo</Badge>
                    ) : (
                      <Badge variant="secondary" className="text-xs" data-testid={`badge-disabled-${motor.id}`}>Desativado</Badge>
                    )}
                  </CardTitle>
                  <CardDescription className="text-sm">{motor.description}</CardDescription>
                </div>
              </div>
            </CardHeader>
            <CardContent>
              {motor.enabled ? (
                <div className="space-y-2">
                  {motor.syncActions.length > 0 ? (
                    motor.syncActions.map((action) => (
                      <Button
                        key={action.type}
                        variant="outline"
                        size="sm"
                        ...
                        onClick={() => syncMutation.mutate(action.type)}
                        data-testid={`btn-sync-${motor.id}-${action.type}`}
                      >
                    ...
                    <p className="text-sm text-muted-foreground">Motor ativo - sincronização automática</p>
                  )}
                </div>
              ) : (
                <div className="flex items-center gap-2 text-sm text-muted-foreground">
                  <CloudOff className="w-4 h-4" />
                  Ative este módulo em Admin → Módulos para sincronizar
                </div>
              )}
            </CardContent>
          </Card>
      ...
      <Card data-testid="card-sync-info">
        ...
        <CardContent className="space-y-3 text-sm text-muted-foreground">
          <div className="flex items-start gap-3">
            ...
            <p><strong>SOE é o kernel:</strong> Todos os dados de negócio passam pelo SOE antes de chegar aos motores de execução</p>
          </div>
          <div className="flex items-start gap-3">
            <CheckCircle className="w-4 h-4 text-emerald-500 mt-0.5 shrink-0" />
            <p><strong>Apps consomem o SOE:</strong> Retail, Engenharia e outros apps lêem e escrevem no SOE, que repassa aos motores</p>
          </div>
          <div className="flex items-start gap-3">
            <CheckCircle className="w-4 h-4 text-emerald-500 mt-0.5 shrink-0" />
            <p><strong>Motores executam:</strong> Plus (ERP Laravel), ERPNext, Fisco e BI são motores invisíveis que o SOE orquestra</p>
          </div>
          <div className="flex items-start gap-3">
            <CheckCircle className="w-4 h-4 text-emerald-500 mt-0.5 shrink-0" />
            <p><strong>Módulos controlam:</strong> Cada tenant pode ativar apenas os motores que precisa em Admin → Módulos</p>
          </div>
        </CardContent>
      </Card>
    </div>

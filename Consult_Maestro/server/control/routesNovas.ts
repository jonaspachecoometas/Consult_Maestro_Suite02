...
export function attachSprint45Routes(app: Express) {
  ...
  app.post("/api/control/grupos", ...auth, async (req: any, res) => {
    try {
      const parsed = insertGrupoEmpresarialSchema.parse({ ...req.body, tenantId: req.tenantId });
  ...
  app.post("/api/control/grupos/:id/membros", ...auth, async (req: any, res) => {
    try {
      const parsed = insertGrupoMembroSchema.parse({ ...req.body, tenantId: req.tenantId, grupoId: req.params.id });
  ...
  app.post("/api/control/contabilidade/lancamentos", ...auth, async (req: any, res) => {
    try {
      ...
      const r = await contabSvc.createLancamento({
        cabecalho: { ...parsed.cabecalho, tenantId: req.tenantId, criadoPor: req.user?.id ?? null } as any,
  ...
  app.post("/api/control/conectores", ...auth, async (req: any, res) => {
    try {
      ...
      const parsed = insertConectorSchema.parse({ ...rest, tenantId: req.tenantId });
  ...
  app.post("/api/control/clientes/:clienteId/regime", ...auth, async (req: any, res) => {
    try {
      ...
      const parsed = insertRegimeTributarioSchema.parse({ ...req.body, tenantId: req.tenantId, clienteId: req.params.clienteId });

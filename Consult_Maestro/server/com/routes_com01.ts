...
export function registerCom01Routes(app: Express): void {
  ...
  app.get('/api/com/pedidos', ...auth, async (req: any, res) => {
    try {
      ...
      if (search) {
        params.push('%' + search + '%');
        q += ` AND (so.numero ILIKE $${params.length} OR p.nome_fantasia ILIKE $${params.length})`;
      }
  ...
  app.patch('/api/com/pedidos/:id', ...auth, async (req: any, res) => {
    try {
      ...
      if (!['rascunho', 'em_aprovacao'].includes(order.status)) {
        return res.status(409).json({ ok: false, error: `Pedido no status '${order.status}' não pode ser editado.` });
      }
  ...
  app.post('/api/com/pedidos/:id/parcelas/recalcular', ...auth, async (req: any, res) => {
    try {
      ...
      if (!condicaoId) return res.status(400).json({ ok: false, error: 'Condição de pagamento não informada.' });
      ...
      for (const item of (b.itens ?? [])) {
        await pool.query(
          ...
          [
            ...
            item.unidade, item.quantidade, item.precoUnitario, item.descontoItem ?? 0,

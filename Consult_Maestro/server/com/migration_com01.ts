 ...
 * sale_quotes, sale_quote_items, sale_orders, sale_order_items,
 * sale_order_installments, sale_order_events, soe_numeracao
 */
...
export async function runMigrationCom01(): Promise<void> {
  ...
  try {
    ...
    `);
    await client.query(`CREATE UNIQUE INDEX IF NOT EXISTS uq_sale_quote_tenant_numero ON sale_quotes (tenant_id, numero)`);
    await client.query(`CREATE INDEX IF NOT EXISTS idx_sale_quote_pessoa ON sale_quotes (tenant_id, pessoa_id, status)`);
    ...
    `);
    await client.query(`CREATE UNIQUE INDEX IF NOT EXISTS uq_sale_order_tenant_numero ON sale_orders (tenant_id, numero)`);
    await client.query(`CREATE INDEX IF NOT EXISTS idx_sale_order_pessoa_status ON sale_orders (tenant_id, pessoa_id, status)`);
    await client.query(`CREATE INDEX IF NOT EXISTS idx_sale_order_status_created ON sale_orders (tenant_id, status, created_at DESC)`);
    await client.query(`CREATE INDEX IF NOT EXISTS idx_sale_order_fiscal_doc ON sale_orders (fiscal_doc_id) WHERE fiscal_doc_id IS NOT NULL`);
    ...
    `);
    await client.query(`CREATE INDEX IF NOT EXISTS idx_sale_installment_status ON sale_order_installments (sale_order_id, status)`);
    ...
    `);
    await client.query(`CREATE INDEX IF NOT EXISTS idx_sale_event_order ON sale_order_events (sale_order_id, created_at DESC)`);

    // 7. soe_numeracao (numeração atômica de documentos)
    await client.query(`

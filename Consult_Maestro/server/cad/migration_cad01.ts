 ...
 * Deve ser executada após SOE-00.
 */
...
export async function runMigrationCad01(): Promise<void> {
  ...
  try {
    ...
    `);
    await client.query(`CREATE UNIQUE INDEX IF NOT EXISTS uq_produto_fiscal_tenant_codigo ON produto_fiscal (tenant_id, codigo)`);
    await client.query(`CREATE INDEX IF NOT EXISTS idx_produto_fiscal_ncm ON produto_fiscal (ncm) WHERE ncm IS NOT NULL`);
    await client.query(`CREATE INDEX IF NOT EXISTS idx_produto_fiscal_product_id ON produto_fiscal (product_id) WHERE product_id IS NOT NULL`);
    await client.query(`CREATE INDEX IF NOT EXISTS idx_produto_fiscal_tenant_status ON produto_fiscal (tenant_id, status)`);
    ...
    await client.query(`
      CREATE TABLE IF NOT EXISTS produto_fiscal_tributacao_uf (
        id                    VARCHAR PRIMARY KEY DEFAULT gen_random_uuid(),
        tenant_id             VARCHAR NOT NULL,
        produto_fiscal_id     VARCHAR NOT NULL REFERENCES produto_fiscal(id) ON DELETE CASCADE,
    ...
    await client.query(`
      CREATE UNIQUE INDEX IF NOT EXISTS uq_emitente_empresa
        ...
        WHERE status = 'ativo'
    `);
    await client.query(`CREATE INDEX IF NOT EXISTS idx_emitente_cnpj ON emitentes_fiscal (cnpj)`);
    ...
    `);
    await client.query(`CREATE UNIQUE INDEX IF NOT EXISTS uq_soe_tabela_preco_codigo ON soe_tabelas_preco (tenant_id, codigo)`);
    await client.query(`
    ...
    await client.query(`
      CREATE TABLE IF NOT EXISTS soe_tabela_preco_itens (
        id                    VARCHAR PRIMARY KEY DEFAULT gen_random_uuid(),
        tabela_preco_id       VARCHAR NOT NULL REFERENCES soe_tabelas_preco(id) ON DELETE CASCADE,
    ...
    `);
    await client.query(`CREATE UNIQUE INDEX IF NOT EXISTS uq_soe_condicao_codigo ON soe_condicoes_pagamento (tenant_id, codigo)`);
    await client.query(`
    ...
    await client.query(`
      CREATE TABLE IF NOT EXISTS soe_condicao_parcelas (
        id                    VARCHAR PRIMARY KEY DEFAULT gen_random_uuid(),
        condicao_pagamento_id VARCHAR NOT NULL REFERENCES soe_condicoes_pagamento(id) ON DELETE CASCADE,
    ...
    await client.query(`
      ...
      SELECT
        p.id, p.tenant_id, p.code, p.name, p.description, p.category, p.unit,
        p.cost_price, p.sale_price, p.stock_qty, p.min_stock, p.barcode,
        COALESCE(pf.ncm, p.ncm)                    AS ncm,
        COALESCE(pf.grupo_tributacao_id, p.tax_group_id) AS tax_group_id,
        p.status, p.image_url, p.requires_serial_tracking, p.tracking_type,
        p.default_brand, p.default_model,
        pf.cest, pf.origem,
        pf.id                                       AS produto_fiscal_id,
        pf.controla_lote, pf.descricao_nfe, pf.unidade_tributavel,
        pf.custo_medio, pf.estoque_minimo,
    ...
    console.error("[CAD-01] Erro na migration:", e.message);
    throw e;
  } finally {
    client.release();
  }
